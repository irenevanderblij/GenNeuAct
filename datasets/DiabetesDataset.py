...
class DiabetesDataset(Dataset):
    ...
    def __init__(self, xy, transforms=None):  
        ...
        self.len = xy.shape[0]  # xy is n rows and 9 columns (N, 9), shape[0] outputs N 
        self.x_data = torch.Tensor(xy.iloc[:, 0:xy.shape[1]-1].values)  # xy[:, :-1] Read all data except the last column.
        self.y_data = torch.LongTensor(xy.iloc[:,xy.shape[1]-1].values)  # [-1] Convert the last dimension to array
